...
function HomePage() {
  ...
  const handleFiltrar = (filtros) => {
    if (parseFloat(filtros.precioMin) > parseFloat(filtros.precioMax)) {
      setMensaje(
        "El precio por día mínimo debe ser menor al precio por día máximo"
      );
      setFiltradas([]);
      return;
    }
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5000/api/Publicacion/all")
      .then((response) => {
        ...
        const imagePromises = fetchedPublicaciones.map((pub) =>
          axios
            .get(
              `http://localhost:5000/api/Archivo/byEntidad?entidadId=${pub.idPublicacion}&tipoEntidad=0`
            )
            .then((res) => ({
      ...
      .catch((err) => {
        ...
        setError(
          "No se pudieron cargar las publicaciones. Por favor, inténtalo más tarde."
        );
        setLoading(false);
      });
  }, []);
  ...
  const publicacionesParaMostrar =
    filtradas.length > 0 || mensaje ? filtradas : publicaciones;
            ...
            {publicacionesParaMostrar
              .filter((pub) => {
                if (user?.roles?.includes("Dueño")) {
                  // Dueño: mostrar todas excepto las eliminadas
                  return pub.status !== "Eliminada";
                } else {
                  // Otros roles: mostrar solo las disponibles
                  return (
                    pub.status !== "Eliminada" && pub.status !== "No disponible"
                  );
                }
              })
              .map((pub) => (
                ...
                >
                  {imagenes[pub.idPublicacion] ? (
                    <img
                      src={`data:image/jpeg;base64,${
                        imagenes[pub.idPublicacion].archivoBase64
                      }`}
                      alt="Máquina"
                      style={{
                        width: "100%",
                  ...
                  <div style={{ padding: "15px" }}>
                    <h3 style={{ margin: "0 0 10px 0", fontSize: "1.2em" }}>
                      {/* {pub.maquina?.modelo?.marca?.marca}{" "}
                      {pub.maquina?.modelo?.modelo} */}
                      {pub.titulo}
                    </h3>
                    <p
                      style={{
