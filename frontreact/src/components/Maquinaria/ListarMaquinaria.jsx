...
export default function MaquinariaList() {
  ...
  useEffect(() => {
    ...
    getPermisosEspeciales().then(setPermisos);
  }, [refresh]);
      ...
      .then((data) => {
        const filtradas = data.filter((maq) => {
          return (
            ...
            (!tag ||
              maq.tagsMaquina.some(
                (t) => t.tag.toLowerCase() === tag.toLowerCase()
              )) &&
            (!permisoEspecial ||
              maq.permisosEspeciales.some(
                (p) => p.permiso.toLowerCase() === permisoEspecial.toLowerCase()
              ))
          );
        });
      ...
      <div className="container" style={{ display: "flex", gap: "2rem" }}>
        ...
        <div className="filtros">
          <select
            value={filtros.marca}
            onChange={(e) =>
              setFiltros({ ...filtros, marca: e.target.value, modelo: "" })
            }
          >
          ...
          <select
            ...
            onChange={(e) =>
              setFiltros({ ...filtros, tipoMaquina: e.target.value })
            }
          >
          ...
          <select
            value={filtros.permisoEspecial}
            onChange={(e) =>
              setFiltros({ ...filtros, permisoEspecial: e.target.value })
            }
          >
            <option value="">Seleccionar permiso especial</option>
            {permisos.map((p, i) => (
              <option key={i} value={p.permiso}>
                {p.permiso}
              </option>
            ))}
          </select>
              ...
              <div key={maq.idMaquina} className="card">
                    ...
                    <div className="permisos-container">
                      {maq.permisosEspeciales?.length > 0 ? (
                        maq.permisosEspeciales.map((perm, index) => (
                          <span key={index} className="permiso">
                            {perm.permiso}
                          </span>
                        ))
                      ) : (
                ...
                <div className="button-container">
                  <StyledButton
                    text="Modificar"
                    onClick={() =>
                      navigate(`/ModificarMaquinaria/${maq.idMaquina}`)
                    }
                  />
                  <StyledButton
                    text="Eliminar"
